...
struct spec {
    ...
    static constexpr auto acceptable_range(id format) { return std::pair{format, format}; }
};
...
struct range_spec : spec {
    ...
    static constexpr std::pair<id, id> acceptable_range(id format) { return {format, format + SPAN }; } 
};
...
template <type_t TYPE, std::derived_from<spec> SPEC, id BASE_ID>
struct traits
{
